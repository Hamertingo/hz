//! mcode's `session/request_permission` onto hz's permission card.
//!
//! **The options are the agent's.** ACP names each one with an `optionId` and a
//! `kind`, and the button carries the id back untouched inside the outcome
//! envelope — so a decision hz never composes is one it can never compose
//! wrongly.

use crate::events::{PermissionBehavior, PermissionOption, PermissionOptionKind};
use crate::harness::permissions::{PendingRequest, Reply, ResolvedOption};
...
/// One button per option the agent offered, in the order the card reads: allow
/// first, standing grants in the middle, refusal last. A kind this build cannot
/// spell is dropped rather than drawn generically — an unspellable grant
/// offered as an ordinary Allow would decide more than the reader asked for.
        ...
        .filter_map(|choice| {
            ...
            Some(ResolvedOption {
                option: PermissionOption {
                    id: format!("mcode-{}", choice.option_id),
                    // The agent's own words: "Allow once", "Allow for this
                    // session", "Reject".
...
mod tests {
    use super::*;
    use crate::harness::mcode::parser::{PermissionChoice, ToolCallRef};
    ...
    /// The three options ACP offers, drawn in card order with the agent's ids
    /// carried back whole inside the outcome envelope.
    #[test]
    fn the_offered_options_become_three_buttons() {
        let request = request(&[
        ...
        assert_eq!(
            pending.options["mcode-allow_once"].decision.as_ref().unwrap(),
    ...
    fn a_request_with_no_refusal_gets_one() {
        ...
        assert_eq!(
            pending.options["mcode-cancelled"].decision.as_ref().unwrap(),
