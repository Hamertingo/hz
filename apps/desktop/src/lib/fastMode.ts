...
/// - `none` — pi and omp, neither of which has a route to one.
...
///
/// Known ceiling: something that opens a fresh `init` mid-turn — a compaction,
/// on a harness that emits one — lands after a refusal and clears the notice
/// until the next refusal re-raises it. Left alone deliberately — the stop is
/// aligned with what the CLI emits against, which is the turn it opened and not
/// the prompt the reader typed, and a boundary of our own would be a second
/// answer to a question the emitter has already answered.
///
