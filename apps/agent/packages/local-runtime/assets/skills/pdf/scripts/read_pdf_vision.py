...
reads __MAVIS_RUNTIME_PORT first, then the active Hz Agent data directory's daemon.port file
name (fallback 5321).
"""
...
def _runtime_call_matrix_tool(tool: str, args: dict) -> dict:
    ...
    try:
        ...
        die(
            ...
            f"Restart the Hz Agent app, or set __MAVIS_RUNTIME_PORT to the runtime port."
        )
