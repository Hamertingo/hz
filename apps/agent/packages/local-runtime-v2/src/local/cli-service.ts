...
import type { ConversationSteerInput } from "@hz/conversation-contract";
import type {
  ProcessLocalContext,
...
export interface CliServiceOptions {
  ...
  /// The Agent owner — the definitions a Session is started *under*, and the
  /// vocabulary a client's Agents screen manages.
  ///
  /// **Not on `applications`.** That aggregate is the Session and Queue use
  /// cases; the Agent owner is composed beside them and reaches here through the
  /// same `RuntimeServices` object the CLI service is built from.
...
export class CliService {
  ...
  setSkillEnabled(
    input: Parameters<LocalRuntimeApplication["skills"]["setSkillEnabled"]>[0],
    enabled: boolean,
  ) {
    return this.options.application.skills.setSkillEnabled(input, enabled);
  }

  getSkill(
    input: Parameters<LocalRuntimeApplication["skills"]["getSkill"]>[0],
    options?: Parameters<LocalRuntimeApplication["skills"]["getSkill"]>[1],
  ) {
    return this.options.application.skills.getSkill(input, options);
  }

  /// The Agent definitions this machine holds, the built-in roles included.
  ///
  /// **A definition, not a Session.** An Agent is a stored identity and prompt
  /// that a Session is later started *under*; this says nothing about what is
  /// running, which is a different question with a different answer.
  listAgents(input: AgentListOptions = {}): Promise<readonly AgentView[]> {
    return this.options.agent.list(input);
  }

  /// One Agent with its stored prompt, or `undefined` for one that is gone.
  ///
  /// `includeContent` is what carries `systemPrompt` and `persona`, and it is
  /// asked for on this read alone: a listing that carried every prompt would be
  /// a payload of whole documents for a screen drawing names.
  getAgent(name: string) {
    return this.options.agent.get(name, { includeContent: true });
  }

  /// Writes a new Agent definition down, and answers what was stored.
  ///
  /// **Definition-only**, the same path the desktop's own create takes: no Root
  /// Session, no greeting. A Session is started later, explicitly, by whoever
  /// wants to talk to it.
  ///
  /// **Without the content review, because the client that reaches this holds no
  /// reviewer.** The review is a managed-gateway call that fails closed, so a
  /// process-local client running on its own provider keys has no verdict to
  /// offer and every write would be refused. See
  /// [`createDefinitionUnreviewed`](AgentApplication) — the reviewed paths are
  /// untouched and a client that *can* review still should.
  createAgent(input: AgentCreateInput) {
    return this.options.agent.createDefinitionUnreviewed(input);
  }

  /// Rewrites an Agent's identity and prompt. Absent fields are left as stored.
  updateAgent(input: AgentUpdateInput) {
    return this.options.agent.updateUnreviewed(input);
  }

  /// Removes an Agent definition.
  deleteAgent(requestRef: string): Promise<void> {
    return this.options.agent.delete(requestRef);
  }
  ...
  /// The reader's own MCP store — the half that is written to, and the half a
  /// screen with switches in it draws.
  ///
  /// **Apart from `listMcpServers` above, which answers a different question.**
  /// That one is what a *session* can reach, built-ins and the project's own file
  /// included, each with a live status; a server switched off does not appear in
  /// it at all. These seven are the file on this machine: every server written
  /// down, off ones included, plus the writes and the one operation that reaches
  /// the network.
  async listConfiguredMcpServers(keyword?: string) {
    return this.requireCapability("mcp", "MCP").listConfiguredServers(keyword);
  }

  async getConfiguredMcpServer(name: string) {
    return this.requireCapability("mcp", "MCP").getConfiguredServer(name);
  }

  async createConfiguredMcpServer(
    name: string,
    input: Parameters<
      NonNullable<LocalRuntimeApplication["mcp"]>["createConfiguredServer"]
    >[1],
    enabled?: boolean,
  ) {
    return this.requireCapability("mcp", "MCP").createConfiguredServer(
      name,
      input,
      enabled,
    );
  ...
  async updateConfiguredMcpServer(
    name: string,
    input: Parameters<
      NonNullable<LocalRuntimeApplication["mcp"]>["updateConfiguredServer"]
    >[1],
    enabled?: boolean,
  ) {
    return this.requireCapability("mcp", "MCP").updateConfiguredServer(
      name,
      input,
      enabled,
    );
  ...
  async deleteConfiguredMcpServer(name: string) {
    return this.requireCapability("mcp", "MCP").deleteConfiguredServer(name);
  }

  async setConfiguredMcpServerEnabled(name: string, enabled: boolean) {
    return this.requireCapability("mcp", "MCP").setConfiguredServerEnabled(
      name,
      enabled,
    );
  }

  async testConfiguredMcpServer(name: string) {
    return this.requireCapability("mcp", "MCP").testConfiguredServer(name);
  }
