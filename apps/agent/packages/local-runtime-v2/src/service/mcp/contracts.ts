...
import type { McpConnectionPool } from '@hz/mcp/runtime/connection-pool';
import type { McpConnectionTokenOverrides, McpToolCallResult } from '@hz/mcp/runtime/types';
...
export interface ConfiguredMcpConnectionTestResult {
  success: boolean;
  toolCount?: number;
  /// What the server answered with, minus the input schemas.
  ///
  /// **The test already listed them** — it connects to count them — so handing
  /// them back costs nothing and saves a caller a second connection to answer the
  /// obvious next question. Names and descriptions only: a schema per tool is a
  /// payload nothing drawing a list reads, and `listToolsLive` still answers the
  /// whole thing.
