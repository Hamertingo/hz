...
import { McpNameRegistry } from '@hz/mcp';
import type { RuntimeTool, ToolResult } from '@hz/agent-core/tools';
...
import type { McpServerLookup } from '@hz/mcp/runtime/connection-pool';
import type { McpConnectionTokenOverrides, ResolvedMcpServer } from '@hz/mcp/runtime/types';
import { MCP_DEFAULTS } from '@hz/mcp/runtime/types';
import {
    ...
    try {
      ...
      return {
        ...
        tools: tools.map((tool) =>
          tool.description ? { name: tool.name, description: tool.description } : { name: tool.name },
        ),
      };
    } catch (error) {
