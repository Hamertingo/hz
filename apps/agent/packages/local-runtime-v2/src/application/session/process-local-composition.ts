...
export function composeProcessLocalApplication(input: {
  ...
  return createProcessLocalApplication({
    ...
    skills: {
      ...
      setSkillEnabled: (request, enabled) =>
        input.compatibility.skills.setSkillEnabled(request, enabled),
      getSkill: (request, options) =>
        input.compatibility.skills.getSkill(request, options),
    },
