import type {
  GlobalEvent,
  GlobalThreadGoal,
} from "@hz/shared/global-events";
import type { ThreadGoalAttachment, ThreadGoalPatchInput } from "@hz/goal";
import type {
  AskQuestionnaireReplyAnswer,
  AskQuestionnaireRequest,
} from "@hz/shared/questionnaire";
import type { LocalSkillService } from "@hz/local-runtime";
...
import type { MiniAppSurfaceSummary } from "@hz/shared/miniapp-surface";
...
export interface LocalRuntimeApplication {
  ...
  readonly skills: Pick<LocalSkillService, "listSkills" | "setSkillEnabled" | "getSkill"> & {
    listRuntimeSkills(
  ...
  readonly mcp?: Pick<
    ...
    | "clearSessionServers"
    | "inspectProjectMcp"
    // The reader's own store rather than what a session can reach: the rows a
    // screen with switches in it needs, the one read that carries configuration,
    // and the writes. See the facade's own note for why the two lists are apart.
