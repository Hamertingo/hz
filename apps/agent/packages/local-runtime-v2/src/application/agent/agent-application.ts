...
import { isDefaultAgentAvatarMarker } from '@hz/shared/agent-avatar';
import {
...
/// A pure string transform, and the one place the canonical file's shape is
/// known — the alternative was a second copy of it living here.
import { patchCanonicalMarkdown } from '../../service/agent/storage/canonical-agent-patch.js';
...
export class AgentApplication {
  ...
  /**
   * Writes a definition without the content review.
   *
   * **The review is a check the caller cannot run, not one it is skipping.** It
   * is a call to the managed gateway and it fails closed on purpose — a gateway
   * refusal, an unreachable gateway or a missing token all block, because treating
   * an unauthenticated caller as approved would let it past every content gate. So
   * on a machine that is not signed in to the managed account *every* config write
   * is refused, and a client that ships its own provider keys — which is the whole
   * population of one — could never write an Agent at all.
   *
   * **The door sits beside the reviewed one rather than inside it.** Every path
   * that can review still does: [`createDefinition`](Self::createDefinition),
   * [`update`](Self::update), `putConfigDocument` and the import path are
   * untouched. A caller reaches for these two only when it knows it holds no
   * reviewer, which is a fact about the client rather than about the content.
   */
  async createDefinitionUnreviewed(input: AgentCreateInput): Promise<AgentView> {
    const created = await this.options.service.create(input);
    publishBestEffort(this.options.publish ?? (() => undefined), {
      type: 'agent.created',
      payload: { agentName: created.exactOwnerName },
  ...
  /// [`update`](Self::update) without the content review — see
  /// [`createDefinitionUnreviewed`](Self::createDefinitionUnreviewed) for why the
  /// door exists at all.
  ///
  /// **The prompt is not an identity field and cannot ride the row.** For a Custom
  /// Agent it lives in the canonical `agent.md`, and the store refuses to edit it
  /// any other way — "Custom Agent systemPrompt must be edited in canonical
  /// agent.md" — so it is applied through the document instead: the existing
  /// content is patched (frontmatter and every unknown field kept) and written
  /// back under the revision that was read. A built-in keeps its bundled prompt
  /// and only its identity moves.
  async updateUnreviewed(input: AgentUpdateInput): Promise<AgentView> {
    const { systemPrompt, ...identity } = input;
    const view = await this.options.service.update(identity);
    if (systemPrompt === undefined || systemPrompt === null) return view;

    const document = await this.options.service.getConfigDocument(
      `agent:${view.exactOwnerName}`,
    );
    if (document.ownerKind !== 'custom') return view;

    const content = patchCanonicalMarkdown(
      document.content,
      view.exactOwnerName,
      { systemPrompt },
      undefined,
    );
    await this.options.service.putConfigDocument({
      requestRef: `agent:${view.exactOwnerName}`,
      content,
      expectedRevision: document.revision,
      ...(document.ownerInstanceId
        ? { expectedOwnerInstanceId: document.ownerInstanceId }
        : {}),
    });
    return this.options.service.get(view.exactOwnerName);
  }
