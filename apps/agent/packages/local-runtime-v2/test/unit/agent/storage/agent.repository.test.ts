...
} from "@hz/local-runtime";
import { DeferredRuntimeConversation } from "@hz/conversation-contract";
import { afterEach, describe, expect, it, vi } from "vitest";
import { formatDefaultAgentAvatarMarker } from "@hz/shared/agent-avatar";
import { serializeAgentReference } from "@hz/shared/agent-mention";
  ...
  it.each(["tui", "coding", "work"] as const)(
    ...
    async (promptMode) => {
      ...
      expect(profile.corePrompt).toContain(
        promptMode === "tui"
          ? "You are a coding agent running in the Hz Agent terminal"
          : "You run inside Hz Agent",
      );
      ...
      expect(frozen.corePrompt).not.toContain(
        "user's active Hz Agent terminal conversation",
      );
