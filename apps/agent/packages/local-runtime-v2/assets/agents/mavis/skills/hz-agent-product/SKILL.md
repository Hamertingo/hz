---
name: hz-agent-product
description: >-
  Use this skill to route questions about the Hz Agent or Mavis product itself:
  ...
  pricing, quotas; and Hz Agent image, audio, music, or video capabilities.
  Treat references such as "Hz Agent", "Mavis", "mcode", "mcode tui", or
...
descriptions:
  zh-Hans: >-
    这是所有 Hz Agent 或 Mavis 产品问题的首层总路由 Skill。凡是询问产品身份、归属、
...
# Hz Agent product identity and routing

## Stable facts

- Hz Agent is MiniMax's agentic coding workspace; Mavis is its primary Agent.
- The public surfaces are Desktop/Electron, Web/H5, and Hz Agent CLI/TUI.
...
- `region: cn` means the current app/account is China. Prefer China-region Hz Agent and Open
  Platform official documentation.
- `region: en` means the current app/account is Global. Prefer Global-region Hz Agent and Open
  Platform official documentation.
...
| Runtime region | Hz Agent product/workflow/Agent/extension docs | Open Platform account/model/API/billing docs  |
| -------------- | -------------------------------------------------- | --------------------------------------------- |
...
- Product identity, Desktop/Web/CLI/TUI, workflows, Agents, Skills, Plugins, and MCP →
  current-region Hz Agent index.
- Token Plan, credits, pricing, API keys, model catalog, model API, and media API → current-region
  Open Platform index; use the current-region Hz Agent index too when the question concerns Code
